...
class Flatten(nn.Module):
    def forward(self, x):       # convert a packed padded sequence to a variable
       return x.data
...
class Layered_RNN(nn.Module):
    def __init__(self, rnn_input_size = 40, rnn_type=nn.LSTM, rnn_hidden_size=768, nb_layers=5, bidirectional=True, batch_norm = True, num_classes = 40):
        ...
        if batch_norm:
           self.fc = nn.Sequential(
               ...
               nn.Linear(num_directions * rnn_hidden_size, num_classes)
           )
        else:
           self.fc = nn.Sequential(
               Flatten(),
